...
def S1_INT_proc(infiles, out_dir= None, tmpdir= None, t_res=20, t_crs=32633,  out_format= "GeoTIFF", gpt_paras= None, pol= 'full', IWs= ["IW1", "IW2", "IW3"], ext_DEM= False, ext_DEM_noDatVal= -9999, ext_Dem_file= None, msk_noDatVal= False, ext_DEM_EGM= True, imgResamp= "BICUBIC_INTERPOLATION", demResamp= "BILINEAR_INTERPOLATION", speckFilter= "Boxcar", filterSizeX= 5, filterSizeY= 5, ml_RgLook= 4, ml_AzLook= 1, l2dB_arg= True, firstBurstIndex= None, lastBurstIndex= None, clean_tmpdir= True):
    ...
    for i in range(0, len(pair_dates_idx)):
        ...
        for p in pol:
            ...
            if len(IWs) == 1:
                out_name= sensor+"_" + p + "_"+IWs[0]+"_INT_relOrb_"+ str(relOrb) + "_"+                    unique_dates_info[i]+"_Orb_Cal_Deb_ML_TF_Spk_TC"
            elif len(IWs)== 2:
                separator = "_"
                iw_str= separator.join(IWs)
                out_name= sensor+"_" + p + "_"+iw_str+"_INT_relOrb_"+ str(relOrb) + "_"+                    unique_dates_info[i]+"_Orb_Cal_Deb_ML_TF_Spk_TC"
            else:
                out_name= sensor+"_" + p +"_INT_relOrb_"+ str(relOrb) + "_"+                    unique_dates_info[i]+"_Orb_Cal_Deb_ML_TF_Spk_TC"
