...
def S1_InSAR_coh_proc(infiles, out_dir= None, tmpdir= None, t_res=20, t_crs=32633,  out_format= "GeoTIFF",gpt_paras= None, pol= 'full', IWs= ["IW1", "IW2", "IW3"], ext_DEM= False, ext_DEM_noDatVal= -9999, ext_Dem_file= None, msk_noDatVal= False, ext_DEM_EGM= True, BGC_demResamp= "BICUBIC_INTERPOLATION", TC_demResamp= "BILINEAR_INTERPOLATION", cohWinRg= 11, cohWinAz= 3, ml_RgLook= 4, ml_AzLook= 1, firstBurstIndex= None, lastBurstIndex= None, clean_tmpdir= True):
    ...
    for i in range(0, len(pair_dates_idx)-1):
        ...
        ##extract dates as str from filename
        ...
        for p in pol:
            for iw in IWs:
                ...
                aof.parameters["orbitType"]= "Sentinel Precise (Auto Download)"
                ...
                aof2.parameters["orbitType"]= "Sentinel Precise (Auto Download)" #'Sentinel Restituted (Auto Download)' Sentinel Precise (Auto Download)
            ...
            if len(IWs) == 3:
                tpm_name= "S1_relOrb_"+ str(relOrbs[0])+"_COH_"+ p + "_"+ date2+"_"+ date1
            else:
                separator = "_"
                iw_str= separator.join(IWs)
                tpm_name= "S1_relOrb_"+ str(relOrbs[0])+"_COH_"+ iw_str+"_"+ p + "_"+ date2+"_"+ date1
            ##create default output folder
            if out_dir is None:
                outdir= "COH/"+p
                if os.path.isdir(out_dir) == False:
                    os.makedirs(os.path.join(os.getcwd(), out_dir))
            final_out_fp=os.path.join(outdir, tpm_name)
