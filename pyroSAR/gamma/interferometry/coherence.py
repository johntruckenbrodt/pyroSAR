...
tab1.write(
    date1 + "_iw1_" + pol + "_slc " + date1 + "_iw1_" + pol + "_slc.par " + date1 + "_iw1_" + pol + "_slc.tops_par\n")
tab1.write(
    date1 + "_iw2_" + pol + "_slc " + date1 + "_iw2_" + pol + "_slc.par " + date1 + "_iw2_" + pol + "_slc.tops_par\n")
tab1.write(
    date1 + "_iw3_" + pol + "_slc " + date1 + "_iw3_" + pol + "_slc.par " + date1 + "_iw3_" + pol + "_slc.tops_par\n")
tab1.close()

tab2 = open("slc2_tab.txt", "w")
tab2.write(
    date2 + "_iw1_" + pol + "_slc " + date2 + "_iw1_" + pol + "_slc.par " + date2 + "_iw1_" + pol + "_slc.tops_par\n")
tab2.write(
    date2 + "_iw2_" + pol + "_slc " + date2 + "_iw2_" + pol + "_slc.par " + date2 + "_iw2_" + pol + "_slc.tops_par\n")
tab2.write(
    date2 + "_iw3_" + pol + "_slc " + date2 + "_iw3_" + pol + "_slc.par " + date2 + "_iw3_" + pol + "_slc.tops_par\n")
tab2.close()
...
diff.gc_map(MLI_par=date1 + ".mli.par", OFF_par="-", DEM_par=dem_name + "_par", DEM=dem_name, DEM_seg_par="EQA.dem_par",
            ...
            lat_ovr='3', lon_ovr='3', sim_sar='-', u='-', v='-', inc=date1 + '.inc', psi='-', pix='-',
            ls_map=date1 + '.ls_map',
            frame='-', ls_mode='-', r_ovr='-', logpath=None, outdir=None,
            shellscript=None)  # frame = 8; ls_mode = 2 in default
...
diff.pixel_area(MLI_par=date1 + ".mli.par", DEM_par="EQA.dem_par", DEM="EQA.dem", lookup_table=date1 + ".lt",
                ...
                pix_gamma0='-', nstep=n_step, area_fact='-', sigma0_ratio='pix', gamma0_ratio='-'
                # nstep = default = 10 passt für unsere Anwendung
                , logpath=None, outdir=None, shellscript=None)
...
print("""### COREGISTRATION ###\n
          ...""")
diff.S1_coreg_TOPS(SLC1_tab="slc1_tab.txt", SLC1_ID=date1, SLC2_tab="slc2_tab.txt", SLC2_ID=date2,
                   RSLC2_tab="slc3_tab.txt",
