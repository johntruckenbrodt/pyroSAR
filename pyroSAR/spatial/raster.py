...
#                    nat = np.asscalar(npn)
    ...
    @classmethod
    def to_array(cls, filename=None, extension=None, path=None, subset_x=None, subset_y=None):
        """Import grid to array       
        ...
        path:            str (default = None)
                         Path to files. If path is None the path will set to current 
                         directory with os.getwd().
        
        subsets:         tuple with lists
        ...
        extension:       str
                         ...
                         spezific extension, set filename to None and define an
                         extension.
        Returns
        -------
        ...
        if ((filename is None and extension is None) or
           (filename is not None and extension is not None)):
            raise KeyError("You must define a filename OR a extension")
        
        elif filename is None and test_string(extension) is False:
            raise KeyError("Extension must be a str. For example '.bin'")

        else:
            if filename is None and extension is not None:
                for root, dirs, files in os.walk(gwd):
                    ...
                    filename = tuple(filename_list)
            else:
                pass

            # Test Data
            string_check, tuple_check, tuple_str_check = test_import_type(filename)

            if not tuple_check and not string_check:
        ...
        if tuple_check:
            inds = tuple(map(lambda x: gdal.Open(x, GA_ReadOnly), filename))

            for i in range(len(inds)):
                print "START: loading %s" %filename[i]
                if inds[i] is None:
                    raise AssertionError("Couldn't open file %s. Perhaps you need an .hdr file?"
                                         %filename[i])
                else:
                    pass

            cols = tuple(map(lambda item: item.RasterXSize, inds))
            rows = tuple(map(lambda item: item.RasterYSize, inds))
            band = tuple(map(lambda item: item.GetRasterBand(1), inds))
            bands = tuple(map(lambda item: item.RasterCount, inds))
            trans = tuple(map(lambda item: item.GetGeoTransform(), inds))
            proj = tuple(map(lambda item: item.GetProjection(), inds))

            origin_x = tuple(map(lambda item: item[0], trans))
            origin_y = tuple(map(lambda item: item[3], trans))
            pixel_width = tuple(map(lambda item: item[1], trans))
            pixel_height = tuple(map(lambda item: item[5], trans))

            arrays = tuple(map(lambda tband, tcols, trows: tband.ReadAsArray(0, 0, tcols, trows),
                               band, cols, rows))

            size = tuple(map(lambda item: item.size, arrays))

            if subset_x is not None and subset_y is not None:
                arrays = tuple(map(lambda item: subset(item, x=subset_y, y=subset_x), arrays))
                cols = tuple(map(lambda item: item.shape[1], arrays))
                rows = tuple(map(lambda item: item.shape[0], arrays))
                size = tuple(map(lambda item: item.size, arrays))

            for item in arrays:
                nan_to_num(item)

            data_to_test = SpatialResults(filename=arrays,
                                          shape=(cols, rows),
                                          size=size)

            test_data(data_to_test, verbose=1)

            result = SpatialResults(data=arrays,
                                    shape=(rows[0], cols[0]),
                                    filename=filename,
                                    size=size[0],
                                    geotransform=trans[0],
                                    projection=proj[0],
                                    pixel_width=pixel_width[0],
                                    pixel_height=pixel_height[0],
                                    origin_x=origin_x[0],
                                    origin_y=origin_y[0],
                                    dataset=inds[0],
                                    bands=bands[0],
                                    np_dtype=arrays[0].dtype.name,
                                    gdal_dtype=TYPEMAP[arrays[0].dtype.name],
                                    working_dir=gwd)

            for item in filename:
                print "END: %s opened successfully" %item

        else:
            inds = gdal.Open(filename, GA_ReadOnly)

            print "START: loading %s" %filename
            if inds is None:
                raise AssertionError("Couldn't open file %s. Perhaps you need an .hdr file?" %filename)
            else:
                pass
            ...
            origin_y = trans[3]
            pixel_width = trans[1]
            pixel_height = trans[5]

            arrays = band.ReadAsArray(0, 0, cols, rows)
            size = arrays.size

            if subset_x is not None and subset_y is not None:
                arrays = subset(arrays, x=subset_x, y=subset_y)
                cols = arrays.shape[1]
                rows = arrays.shape[0]
                size = arrays.size
            ...
            print "END: %s opened successfully" %filename
            ...
            result = SpatialResults(data=arrays,
                                    ...
                                    dataset=inds,
                                    bands=bands,
                                    np_dtype=arrays.dtype.name,
                                    gdal_dtype=TYPEMAP[arrays.dtype.name],
                                    working_dir=gwd)
